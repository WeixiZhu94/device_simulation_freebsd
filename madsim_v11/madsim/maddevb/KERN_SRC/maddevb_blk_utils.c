...
static bool maddevb_setup_fault(void);
static void maddevb_setup_bwtimer(struct maddevb *maddevb);
static void maddevb_cleanup_queues(struct maddevb *maddevb);
...
static int maddevb_handle_flush(struct maddevb *maddevb)
{
	...
	if (err != 0)
		{PERR("maddevb_handle_flush... dev=%ld err=%d\n",
...
static int maddevb_handle_req(struct maddevb_cmd *cmd)
{
	...
	PINFO("maddevb_handle_req dev#=%d cmd=%px req=%px sector=%lld\n", 
							...
							enum req_opf op)
{
	...
	int	  iorc = 0;
	int	  nr_bvecs = 0;
	struct bio_vec *biovecs;
	// struct   bio_vec biovecs[MAD_SGDMA_MAX_SECTORS];
	ssize_t  iocount;

	biovecs = kzalloc(sizeof(struct bio_vec) * MAD_SGDMA_MAX_SECTORS, GFP_KERNEL);
	if (biovecs == NULL)
		return -ENOMEM;
	
	mutex_lock(&pmaddevobj->devmutex);

	PINFO("maddevb_process_bio_command... dev#=%ld op=%d bio=%px bio_next=%px #biovecs=%d biovecs=%px\n",
				   ...
				   sector_t nr_sectors, enum req_opf op)
{
	...
	PINFO("maddevb_handle_cmd... dev#=%ld cmd=%px req=%px bio=%px sector=%d #sctrs=%d op=%d\n",
	...
	switch (op)
		...
		default:
			PWARN("maddevb_handle_cmd... dev#=%ld cmd=%px req=%px op=%d sts=BLK_STS_NOTSUPP\n",
...
void maddevb_cleanup_queues(struct maddevb *maddevb)
{
	...
	PINFO("maddevb_cleanup_queues... mbdev#=%ld dev=%px #Qs=%d\n",
...
void maddevb_delete_device(struct maddevb *maddevb)
{
	...
	PDEBUG("maddevb_delete_device... dev#=%ld mbdev=%px maddevb=%px\n",
	...
	PINFO("maddevb_delete_device exit... mbdev#=%ld dev=%px maddevb=%px\n",
		  pmaddevobj->devnum, mbdev, maddevb);
}
...
static struct maddevblk_device *maddevb_alloc_device(PMADDEVOBJ pmaddevobj)
{
	...
	PINFO("maddevblk_alloc_device... mbdev#=%ld dev=%px\n",
...
static int maddevb_gendisk_register(struct maddevb *maddevb)
{
	...
	PINFO("maddevb_gendisk_register... dev#=%ld maddevb=%px\n",
...
static int maddevb_add_device(struct maddevblk_device *mbdev)
{
	...
	switch (mbdev->queue_mode)
		...
		default:
			PWARN("maddevb_add_device... mbdev#=%ld unsupported Qmode=%d\n",
							...
							struct page *pPage, unsigned int op)
{
	...
	PINFO("maddevb_rw_page... dev#=%ld bdev=%px PA=x%llX sector=%ld op=%d\n",
