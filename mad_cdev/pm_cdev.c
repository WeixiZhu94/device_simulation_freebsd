...
vmem_t *pm_pool;
vm_page_t first_page, last_page;
size_t npages = 1024 * 1024 / 4;

int init_pm(gmem_mmu_ops *ops) {
    first_page = vm_page_alloc_contig(NULL, 0, VM_ALLOC_NORMAL | VM_ALLOC_NOBUSY | VM_ALLOC_NOOBJ,
    ...
    if (first_page == NULL)
    	...
    	for (int i = 0; i < npages; i ++)
    		first_page[i].flags |= PG_NOCPU;
        last_page = &first_page[npages - 1];
        ops->pa_min = VM_PAGE_TO_PHYS(first_page);
        ops->pa_max = VM_PAGE_TO_PHYS(last_page);
...
vm_page_t alloc_pm(void *)
{
	...
	if (vmem_alloc(pm_pool, 1, &page_idx) == 0) {
		pmap_zero_page(&first_page[page_idx]);
		return &first_page[page_idx];
	}
	else
		return NULL;
}
