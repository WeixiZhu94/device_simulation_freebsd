...
static char *cm_devnode(struct device *dev, umode_t *mode)
{
    if (mode)
        *mode = 0666;
    return kasprintf(GFP_KERNEL, "madbus/%s", dev_name(dev));
}
...
static U8     bDrvSysfs[MADBUS_NUMBER_SLOTS] = {0};
...
pcisim_get_device(unsigned int vendor, unsigned int pci_devid, struct pci_dev *start)
{
    ...
    //
    printf("%s : %d\n", __func__, __LINE__);
    struct pci_dev* ret = madbus_setup_pci_device((indx+1), pci_devid);
    printf("%s : %d\n", __func__, __LINE__);
    return ret;
}
//
...
static inline void madbus_init_pcidev(PMADBUSOBJ pmadbusobj)
{
    ...
    // abuse the class field to save this
    pPciDev->class = pmadbusobj->devnum;
...
struct pci_dev* madbus_setup_pci_device(U32 indx, U16 pci_devid)
{
    ...
    printf("[!!!] Using madbus_objects, 0x%p, index %lu\n", madbus_objects, indx);
