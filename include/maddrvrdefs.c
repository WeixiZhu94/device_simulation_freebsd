...
static struct kobject mad_kset;
...
maddev_setup_device(PMADDEVOBJ pmaddevobj, struct pci_dev** ppPciDvTmp, U8 bHPL)
{
    ...
    if (!bHPL)
    {
        *ppPciDvTmp = pcisim_get_device(MAD_PCI_VENDOR_ID, MAD_PCI_CHAR_DEVICE_ID, *ppPciDvTmp);
        if ((*ppPciDvTmp == NULL) || (IS_ERR(*ppPciDvTmp)))
            {
            mutex_unlock(&pmaddevobj->devmutex);
            PERR("maddev_setup_device... dev#=%d pci_get_device rc=%ld\n",
    ...
    if (rc != 0)
        ...
        maddev_release_pci_resrcs(pmaddevobj);
        mutex_unlock(&pmaddevobj->devmutex);
        PERR("maddev_setup_device:dma_set_mask... dev#=%d rc=%ld\n", (int)i, rc);
        return -ENOTSUP;
        }
    #endif
    ...
    PINFO("maddev_setup_device... dev=%d pdevobj=%px devVA=%px devPA=x%lX exit :)\n",
          (int)i, pmaddevobj, 
...
void maddev_cleanup_module(void)
{
    ...
    pcisim_unregister_driver(&maddev_driver);
...
{
    // not actually used
    int irqflags = 0; // IRQF_TRIGGER_HIGH | IRQF_TRIGGER_LOW;
    register U32 j = 0;
    U8 bMSI = 0; //pPciDev->msi_cap;
    int rc = 0;
    ...
    PDEBUG("pci_read_config_dword... BaseAddr=x%llX PciCnfg_Hi:Lo,64=x%lX:%lX x%llX\n",
    ...
    pmaddevobj->pDevBase = (struct _MADREGS *) phys_to_virt(pmaddevobj->MadDevPA);

    PINFO("maddev_claim_pci_resrcs... dev#=%d PA=x%lX kva=%px\n",
...
int maddev_release_pci_resrcs(PMADDEVOBJ pmaddevobj)
{
    ...
    //     pcisim_disable_msi(pmaddevobj->pPciDev);
    // }
                             ...
                             sector_t sector, bool bWr)
{
    ...
    PINFO("maddev_xfer_dma_page... dev#=%d PA=x%lX sector=%ld wr=%d\n",
                                ...
                                sector_t sector, bool bWr)
{
    ...
    PDEBUG("maddev_xfer_sgdma_pages... dev#=%d num_pgs=%ld wr=%d\n",
           (int)pmaddevobj->devnum, num_pgs, bWr);

    if (num_pgs > MAD_SGDMA_MAX_PAGES)
        {
        PERR("maddev_xfer_sgdma_pages... dev#=%d num_pgs(%ld) > max(%ld) rc=-EINVAL\n",
    ...
    writeq(virt_to_phys((uintptr_t) pSgDmaElement), (void *) &pmaddevobj->pDevBase->BCDPP);
    ...
    for (j=0; j <= loop_lmt; j++)
    {
        ...
        CDPP = (j == loop_lmt) ? MAD_DMA_CDPP_END : 
                                 virt_to_phys((uintptr_t) &pmaddevobj->SgDmaElements[j+1]);
        ...
        if (j < loop_lmt)
            pSgDmaElement = (PMAD_DMA_CHAIN_ELEMENT)phys_to_virt(CDPP);
    }
                                 ...
                                 struct page* page_list[], U32 offset, bool bWr)
{
    ...
    CntlReg = CountBits;
    //
    HostPA = VM_PAGE_TO_PHYS(page_list[0]);
...
NeedSgXit:;
    ...
    PDEBUG("maddev_need_sg... num_pgs=%d PA0=x%llX PAx=x%llX\n",
                              ...
                              phys_addr_t HostAddr, U32 offset, bool bWr)
{
    ...
    PINFO("maddev_program_stream_io... dev#=%d PA=x%lX CntlReg=x%lX IntEnable=x%lX offset=x%lX wr=%d\n",
...
U32 StatusReg; 
    ...
    switch(StatusReg)
        ...
        case MAD_STATUS_NO_ERROR_MASK: //No Status bits set ?!?
            rc = -1;               //Bad exchange - device disagrees w/ itself ?
            break;
