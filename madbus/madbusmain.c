...
static int madbus_dev_setup_cdev(struct madbus_object *mbobj, int indx)
{
    ...
    // change name
    kobject_set_name(&mbobj->cdev_str.kobj, "madbus%d", mbobj->devnum);
...
static int madbus_malloc_device_memory(PMADBUSOBJ pmadbusobj)
{
    ...
    if (rc == 0)
    {
        ...
        memset(pmadbusobj->pmaddevice, 0x0, MAD_DEVICE_MEM_SIZE_NODATA);
...
static int madbus_setup_device(PMADBUSOBJ pmadbusobj)
{
    ...
    pmadbusobj->sysfs_dev.class = &cm_class;
    pmadbusobj->sysfs_dev.driver_data = NULL;

    dev_set_name(&pmadbusobj->sysfs_dev, "madbus:%d", i);
...
MODULE_VERSION(madbus, 1);
MODULE_DEPEND(madbus, linux64, 1, 1, 1);